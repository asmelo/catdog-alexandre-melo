...
- **A lista não é ordenada no cliente.** `adicionar` e `substituir` inserem na posição correta comparando por `nome.toLowerCase()` com comparação binária de string — exatamente o critério do `ORDER BY name_normalized` do servidor. Usar `localeCompare` aqui produziria uma ordem diferente da do backend e a posição do item recém-criado divergiria após o próximo recarregamento (RN-11 / CA-04 / CT-13 / CT-14).
