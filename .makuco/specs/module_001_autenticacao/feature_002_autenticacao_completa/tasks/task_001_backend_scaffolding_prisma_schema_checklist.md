...
- [x] Zero new issues introduced in SonarQube analysis (if applicable).
  - 3 code smells `typescript:S1135` ("Complete the task associated to this TODO comment"), todos severidade **INFO** e **0min** de débito. Nenhum é defeito:
    ...
    - `src/config/env.ts:9` — **falso positivo**: o `textRange` (offset 46–50) aponta para a palavra portuguesa "todo" em *"em todo o backend"*. Como o `MAKUCO.md` exige documentação em PT-BR, S1135 vai reincidir — decidir no quality profile do projeto (desabilitar a regra ou marcar como *won't fix* sistematicamente).
...
- **BLOQUEIO-B — Supabase não provisionado.** Não existe `.env` versionado nem projeto Supabase acessível. Os ACs 3–7 foram validados contra um PostgreSQL 16 descartável em Docker (removido ao final), **não** contra o Supabase: pooler na 6543, `directUrl` na 5432 e limites de conexão permanecem **não verificados** em ambiente real. `prisma migrate deploy` não foi executado contra produção, conforme a restrição.
- **RISCO-C — Node local divergente do contrato.** `engines` declara `>=20 <21` (Node 20 LTS, conforme `stack.md`), mas a máquina roda Node v23.10.0 → `npm warn EBADENGINE` em toda instalação. Build, typecheck e runtime funcionaram, mas o CI deve fixar Node 20 para valer como validação.
- Um `.env` local **temporário** com valores placeholder foi criado em `services/backend/.env` para permitir as validações de schema e boot. Está coberto pelo `.gitignore` (confirmado por `git check-ignore`) e **não** contém credencial real. `DATABASE_URL` aponta para um host inexistente — substitua pelos valores reais do Supabase antes de rodar `npm run prisma:migrate`.
