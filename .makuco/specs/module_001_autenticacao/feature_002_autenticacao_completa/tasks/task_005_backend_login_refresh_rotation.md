...
- [ ] **Given** um refresh token já rotacionado, **When** reapresentado, **Then** `401 SESSION_EXPIRED`, **todos** os tokens daquele `familyId` ficam com `revokedAt != null` — os que ainda estavam ativos recebem `revokedReason = 'REUSE_DETECTED'`, e os já revogados **preservam** o motivo original (o token reapresentado permanece `ROTATED`) — e um log `refresh_token_reuse_detected` é emitido (RN-07 / CT-15).
  > **Nota (corrigido em 2026-08-22, TASK-BACKEND-005):** a redação anterior exigia `revokedReason = 'REUSE_DETECTED'` em *todos* os tokens, o que é incompatível com o `revokeFamily` prescrito na seção *Implementation* (`where: { familyId, revokedAt: null }`): o token que dispara a detecção já está revogado como `ROTATED` e é excluído do filtro por construção. Sobrescrever esse motivo destruiria a informação de que ele foi legitimamente rotacionado, degradando a auditoria exatamente no incidente em que ela mais importa. O que a RN-07 exige — nenhum token da família utilizável — é cumprido por `revokedAt != null` em todos, verificado por execução.
